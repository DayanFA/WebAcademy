...
public class PacienteControllerIntegrationTest {
    ...
    public void testPacienteGetAll() throws Exception {
        mockMvc.perform(MockMvcRequestBuilders.get("/paciente/"))
            ...
            .andExpect(MockMvcResultMatchers.jsonPath("$.content[0].nome", Matchers.is("Giulia Farias Bencatel")))
            .andExpect(MockMvcResultMatchers.jsonPath("$.content[1].nome", Matchers.is("Wallace Macedo Inácio Soriano")))
            .andExpect(MockMvcResultMatchers.jsonPath("$.content[2].nome", Matchers.is("Helen Dutra Vilar")))
            .andExpect(MockMvcResultMatchers.jsonPath("$.content[3].nome", Matchers.is("Jean Schuenck Amorin")))
            .andExpect(MockMvcResultMatchers.jsonPath("$.content[4].nome", Matchers.is("Lucilene Santos Lucas")));
    }
