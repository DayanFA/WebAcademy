...
public class UnidadeControllerIntegrationTest {
    ...
    public void testUnidadeGetAll() throws Exception {
        mockMvc.perform(MockMvcRequestBuilders.get("/config/unidade/"))
            ...
            .andExpect(MockMvcResultMatchers.jsonPath("$.content[0].nome", Matchers.is("Bela Vista")))
            .andExpect(MockMvcResultMatchers.jsonPath("$.content[1].nome", Matchers.is("Bosque")))
            .andExpect(MockMvcResultMatchers.jsonPath("$.content[2].nome", Matchers.is("Cruzeiro do Sul")));
    }
